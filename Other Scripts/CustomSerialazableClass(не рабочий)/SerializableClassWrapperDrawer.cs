...
namespace PropertyOld
{
    ...
    public class SerializableClassWrapperDrawer : PropertyDrawer
    {
        ...
        private SerializedProperty[] GetAdditionalSerializedPropertyWrapper(SerializedProperty property)
        {
            ...
            string defaultPath = property.propertyPath;
            ...
            if (property.NextVisible(false))
            {
                do
                {
                    ...
                    // Надо брать путь первого свойства и проверять есть ли оно в путях всех последующих
                    if (property.propertyPath.Contains(defaultPath) == false) continue;
        ...
        private FieldInfo[] GetVisibleFields(Type type)
        {
            ...
            FieldInfo[] allField = type.GetFields(BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Public);
