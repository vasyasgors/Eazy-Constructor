...
public class EventHandlerEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        //!!!!!!!НУЖНО НУЖНО НУЖНО если ActionContainer монобех  eventHandler = target as ActionContainer;
    ...
    private void AddAction(object name)
    {
        ...
        if(name.ToString() == "Destory Object")
            eventHandler.AddAction<DestoryAction>(gameObject);

        if (name.ToString() == "Spawn Object")
            eventHandler.AddAction<SpawnAction>(gameObject);

        if (name.ToString() == "Load Level")
            eventHandler.AddAction<LoadLevelAction>(gameObject);
        */
